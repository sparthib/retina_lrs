...
dge_output_dir <- "/users/sparthib/retina_lrs/processed_data/dtu/Isoquant/FT_vs_RGC/DGE/"
dte_output_dir <- "/users/sparthib/retina_lrs/processed_data/dtu/Isoquant/FT_vs_RGC/DTE/"
...
FT_RGC_counts <- counts[, c(1, 9, 10, 11, 12)]
FT_RGC_tpm <- tpm[, c(1, 9, 10, 11, 12)]

# Define group factors for FT and RGC
group <- factor(c("FT", "FT", "RGC", "RGC"))

# Create DGEList object and filter
y <- DGEList(counts=FT_RGC_counts, group=group, genes=FT_RGC_counts$gene_id, samples=sample_names[8:11])
...
contr <- makeContrasts(FT_vs_RGC = FT - RGC,
                       levels=design)
...
y <- DGEList(counts=FT_RGC_counts, group=group, genes=FT_RGC_counts$isoform_id, 
             samples=sample_names[8:11])
