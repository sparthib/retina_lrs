...
Variant calling works best with WGS data that has high coverage (> 30x, i.e.
15x per haplotype in the human genome). It can be performed with RNA-Seq data as well, but this is not recommended for reasons such as:
...
One alignment is done, we can use `samtools` to sort the BAM file and index it. 
...
The `ApplyBQSR` command can then be used to apply the recalibrated base quality scores to the BAM file.
...
In order to QC these variants, we need them split into SNPs and INDELs. This is because SNPs and INDELs need to be treated differently during QC. 
 This can be done using the `SelectVariants` command. 

The `VariantFiltration` command can then be used to filter the variants based on various criteria such as quality, depth, and allele frequency.
