...
ora_data <- function(genelist, ont, output_data_dir){
  ego <- enrichGO(gene          = names(genelist),
                  ...
                  qvalueCutoff  = 0.001,
                  readable      = TRUE) 
  write_tsv(as.data.frame(ego), file.path(output_data_dir, paste0("ora_", ont, ".tsv")))
}
...
ora_plots <- function(genelist, ont, output_plot_dir){
  ...
  #dotplot
  pdf(file.path(output_plot_dir, paste0("ora_dotplot_", ont, ".pdf")))
  ...
  #goplot
  pdf(file.path(output_plot_dir, paste0("ora_goplot_", ont, ".pdf")))
