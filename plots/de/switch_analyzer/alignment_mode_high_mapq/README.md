...
from salmon run in alignment mode with `--ont` flag on reads with MAPQ > 30. 
Accompanying `tsv` files in `processed_data/dtu/IsoformSwitchAnalyzer/salmon_alignment_mode_high_mapq`

All gene information found in `DEXSeqSwitchList.tsv` 
Isoform counts and abundance matrices in `isoform_counts.tsv`, and `isoform_abundance.tsv`

Raw gene counts are in `extracted_gene_counts.tsv`
...
`gene_counts_density.pdf`: counts density plots for genes. 

`gene_logCPM_pca.pdf`: PCA plot of top 2 PCs on gene log CPM. 

`gene_logTMM_pca.pdf`: PCA plot of top 2 PCs on isoform log TMM. 

`top_200_isoforms_heatmap.pdf`: Clustering Heatmap on isoforms. 
